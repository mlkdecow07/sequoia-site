...
export default async function AdminContactListPage() {
  ...
  const { data, error } = await supabase
    ...
    .select("id, created_at, name, email, message, source, status")
    .order("created_at", { ascending: false });

  if (error) {
    return (
      <p className="rounded border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
        Unable to load contact submissions: {error.message}
      </p>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <ul className="divide-y divide-teal/10 overflow-hidden rounded border border-teal/15 bg-white">
        {(data ?? []).length === 0 ? (
          <li className="px-4 py-8 text-sm text-gray-500">No submissions yet.</li>
        ) : (
          (data ?? []).map((item) => (
            <li key={item.id}>
              <Link
              ...
              >
                ...
                <p className="mt-2 line-clamp-2 text-sm text-gray-700">{item.message}</p>
                {item.source ? (
