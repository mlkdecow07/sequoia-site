...
export default function HeroGallery({ siteAlert }: { siteAlert?: SiteAlert | null }) {
  ...
  const [alertOpen, setAlertOpen] = useState(Boolean(siteAlert));
  ...
  const showAlert = Boolean(siteAlert) && alertOpen;

  return (
    <>
      <HeroSection images={heroImages}>
        {showAlert && siteAlert ? (
          <SiteAlertBanner alert={siteAlert} onOpenChange={handleAlertOpenChange} />
        ) : (
          <>
